...
sys.path.append(os.path.join(os.path.dirname(__file__), '../..')) # append slim
...
class MobilenetV3Test(tf.test.TestCase):
  ...
  def testCreation(self):
    spec = dict(mobilenet_v3.V3_LARGE_DEF)
    ...
    sess = tf.Session()
    tf.summary.FileWriter('D:/ICF_AutoCapsule_disabled/80_dnn/models-master/research/slim/nets/mobilenet/logs', sess.graph)
    '''
  ...
  def testCreationNoClasses(self):
    spec = copy.deepcopy(mobilenet_v3.V3_LARGE_DEF)
  ...
  def testWithOutputStride8(self):
    out, _ = mobilenet.mobilenet_base(
        ...
        conv_defs=mobilenet_v3.V3_LARGE_DEF,
        output_stride=8,
        scope='MobilenetV3')
  ...
  def testDivisibleByWithArgScope(self):
    ...
    with slim.arg_scope((mobilenet.depth_multiplier,), min_depth=32):
      mobilenet_v3.mobilenet(
          ...
          conv_defs=mobilenet_v3.V2_LARGE_DEF, depth_multiplier=0.1)
      s = [op.outputs[0].get_shape().as_list()[-1] for op in find_ops('Conv2D')]
      s = set(s)
  ...
  def testFineGrained(self):
    ...
    mobilenet_v3.mobilenet(
        tf.placeholder(tf.float32, (10, 224, 224, 2)),
        conv_defs=mobilenet_v3.V2_LARGE_DEF, depth_multiplier=0.01,
  ...
  def testMobilenetBase(self):
    ...
    with slim.arg_scope((mobilenet.depth_multiplier,), min_depth=32):
      net, _ = mobilenet_v3.mobilenet_base(
          tf.placeholder(tf.float32, (10, 224, 224, 16)),
          conv_defs=mobilenet_v3.V2_DEF, depth_multiplier=0.1)
