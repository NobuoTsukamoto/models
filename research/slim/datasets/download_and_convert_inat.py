...
_TRAIN_ASIA_DATA_URL = 'https://storage.googleapis.com/inat_data_2018_asia/train_val2018.tar.gz'
_TRAIN_ASIA_ANNOTATION_URL = 'https://storage.googleapis.com/inat_data_2018_asia/train2018.json.tar.gz'
_VAL_ASIA_ANNOTATION_URL = 'https://storage.googleapis.com/inat_data_2018_asia/val2018.json.tar.gz'
...
def _create_tf_record_from_coco_annotations(
    annotations_file, category_index, image_dir, output_path, target_category, num_shards):
    ...
    """
    with contextlib2.ExitStack() as tf_reord_close_stack, \
        tf.gfile.GFile(annotations_file, 'r') as fid:
...
def _create_label_from_coco_annotations(
    ...
    Returns:
        category index.
    """
    with tf.gfile.GFile(annotations_file, 'r') as fid1, \
        tf.gfile.GFile(un_obfuscated_name_file, 'r') as fid2:
        ground_truth_data = json.load(fid1)
        un_obfuscated_name = json.load(fid2)

        # Get category index from 'categories'
        category_index = _create_category_index(un_obfuscated_name['categories'], target_category)
        ...
        for annotation in annotations_index:
            if annotations_index['category_id'] not in labels:
                ...
                category_index[category_id]['label_index'] = idx
...
def main(_):
    ...
    dataset_utils.download_and_uncompress_tarball(_TRAIN_ASIA_ANNOTATION_URL, annotation_dir)
    dataset_utils.download_and_uncompress_tarball(_VAL_ASIA_ANNOTATION_URL, annotation_dir)
    dataset_utils.download_and_uncompress_tarball(_UN_OBFUSCATED_NAME_URL, annotation_dir)
